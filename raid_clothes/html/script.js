...
$(function () {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (event.data.type == "enableclothesmenu") {
            ...
            if (open) {
                ...
                if (event.data.creator == false) {
                    $('#discard').show();
                } else {
                    if(event.data.showPeds) {
                        $('#button-ped').show();
                    }
                    $('#discard').hide();
                }
                if (event.data.outfits) {
                    ...
                    $('#save-dropdown')
                             .append($('<option>', " ")
                             .text(" "));
                    $.each(event.data.outfits, function(key, value) {
                        $('#save-dropdown')
        ...
        if (event.data.type == "barbermenu") {
            ...
            SetupHeadStructure(event.data.headStructure, event.data.eyeColor);
        }
        ...
        if (open) {
            ...
            if (data.which == 27) {
                if ($('#closemenu').hasClass('open')) {
                    ...
                    if(currentMenu.selector != "#clothesmenu") {
                        $("#save-name").hide();
                    } else if(currentMenu.selector != "#barbermenu" && currentMenu.selector != "#tattoomenu") {
                        $("#save-dropdown").hide();
                        $("#save-name").show();
                    }
                    if(currentMenu.selector == "#barbermenu") {
                        $("#save-dropdown").show();
                    }
                }
            }
    ...
    $('#save').on('click', function() {
        if (currentMenu.selector == "#clothesmenu") {
            CloseMenu(true, $("#save-name").val())
        } else if (currentMenu.selector == "#barbermenu") {
            CloseMenu(true, $("#save-dropdown").val())
        } else {
            CloseMenu(true, "")
        }
    })
    ...
    function CloseMenu(save, name) {
        $.post('https://raid_clothes/escape', JSON.stringify({save:save, name:name}));
    }
    ...
    function SetupHeadStructure(data, eyeColor) {
        ...
        $('[data-value="Eye_Color"]').val(eyeColor)
    }
