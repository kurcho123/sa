...
Padlock.Close = function() {
    ...
    $.post('https://keypad/PadLockClose');
}
...
Keypad.Close = function(data) {
    ...
    $.post('https://keypad/PadLockClose');
    if (data.error != null) {
        $.post('https://keypad/CombinationFail');
    }
}
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post("https://keypad/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('https://keypad/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('https://keypad/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
