...
CreateThread(function()
    for k, v in pairs(Config.MicrophoneZones) do
        ...
        exports['polyzonehelper']:AddBoxZone("microphone_"..v.name, v.coords, v.length, v.width, v.data)
    end
end)
...
AddEventHandler('polyzonehelper:enter', function(ZoneName, ZoneData)
    if string.starts(ZoneName, "microphone") then
        oldProximity =  LocalPlayer.state['proximity'].distance
        exports["pma-voice"]:overrideProximityRange(ZoneData.range, true)
    end
end)

AddEventHandler('polyzonehelper:exit', function(ZoneName, ZoneData)
...
AddEventHandler('onResourceStop', function(resource)
    ...
    for k, v in pairs(Config.MicrophoneZones) do
        exports['polyzonehelper']:DeletePolyZone("microphone_"..v.name)
        if v.obj then
            DeleteEntity(v.obj)
            v.obj = nil
        end
