...
local ESXJobs      = true         --enable ESX   default: false
local Jobs         = {'tow'}    --ESX job names, separated by comma
local PlayerData   = {}
--end ESX --
...
function allowed()
    if (ESXJobs and PlayerData and PlayerData.job and has_value(Jobs,PlayerData.job.name)) or (not ESXJobs) then
        return true
    else
        return false
    end
...
function raiseBed()
	...
	for car in enumerate_vehicles() do
		...
		if #(playerCoords - cvehicleCoords) < Config.InteractDistance then
...
function lowerBed()
	...
	for car in enumerate_vehicles() do
		local cvehicleCoords =  GetEntityCoords(car)
		if GetDistanceBetweenCoords(playerCoords, cvehicleCoords, true) < Config.InteractDistance then
			if validTruck(car) then
				closeCar = car
			end
		end
