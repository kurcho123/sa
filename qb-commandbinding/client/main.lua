        ...
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('There is still nothing ['..v[2]..'] bound, /binds to bind a command', 'inform', 4000)
                end
            else
                QBCore.Functions.Notify('You have not bound any commands, /binds to bind a command', 'inform', 4000)
            end
        end
    end, false)
