...
local function updateUi()
	...
	if not mediaPlayers then mediaPlayers = {} end
		...
		if not duiBrowser then
			...
			if mediaPos and (model or scaleform) then
				...
				if #(viewerPos - mediaPos) < (data.range or Config.maxRange) then
					if data.options and data.options.url then
...
local function invalidateIdleCams()
	if Config.isRDR then
		InvokeNative(0x634F4A0562CF19B8)
	else
		InvalidateIdleCam()
	...
	while true do
		...
		for handle, info in pairs(mediaPlayers) do
			...
			if mediaPos then
				...
				if info.bucket then
					if LocalPlayer.state.bucket ~= info.bucket then 
						distance = 100000.0 -- Just do this to disable
					end
				end
				local camDistance
				...
				if duiBrowser and duiBrowser:isDrawable() then
					...
					local mediaInterior = GetInteriorAtCoords(mediaPos)
					if mediaInterior ~= 0 then
						if isInSameRoom(ped, entity) then
							if distance < info.range then
