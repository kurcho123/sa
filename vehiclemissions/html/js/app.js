    ...
    let Timer = setInterval(() => {
        if (document.getElementById(`timer${index}`) === null) {
            ...
            if (element.started === true) {
                ...
                $.post('https://vehiclemissions/NUI:deletecontract', JSON.stringify({ 'contract': element.id }));
                clearInterval(Timer);
...
const CloseButton = () => {
    $.post('https://vehiclemissions/NUI:close', JSON.stringify({}));
    display = false;
...
const PlayerContract = (id, index, price) => {
    if (crypto - price >= 0) {
        ...
        $.post('https://vehiclemissions/NUI:playercontract', JSON.stringify({ 'index': id }));
        started === true;
    } else {
...
const VinScratch = (id, index, price, vinPrice) => {
    if (crypto - price - vinPrice >= 0) {
        ...
        document.getElementById('accepted-container').innerHTML = '';
        for (let i = 0; i < contractData.length; i++) { document.getElementById(`start-contract${i}`).disabled = true; }
        $.post('https://vehiclemissions/NUI:vinscratch', JSON.stringify({ 'index': id }));
        started === true;
    } else {
        let Div = document.getElementById('insufficient-crypto');
        Div.innerHTML += `
...
const NormalDropOff = (id, index, price) => {
    if (crypto - price >= 0) {
        ...
        document.getElementById('accepted-container').innerHTML = '';
        for (let i = 0; i < contractData.length; i++) { document.getElementById(`start-contract${i}`).disabled = true; }
        $.post('https://vehiclemissions/NUI:normaldropoff', JSON.stringify({ 'index': id }));
        started === true;
    } else {
        let Div = document.getElementById('insufficient-crypto');
        Div.innerHTML += `
...
const Contract = (id, index, price, vinPrice, player) => {
    ...
    if (player == true) {
        ...
        return (
			...
			<p class="accepted-contract-text">If you choose to VIN scratch it will cost an <br> additional ${vinPrice} PDX to claim ownership.</p>
			<div class="accepted-contract-optionButton-flex">
...
const Continue = (contractId) => {
    ...
    elem.remove()
    $.post('https://vehiclemissions/NUI:transfercontract', JSON.stringify({
        targetId: input,
        contract: contractId
    }));
}
...
const ToggleQueue = () => {
    inQueue = !inQueue
    if (inQueue === true) { $.post('https://vehiclemissions/NUI:togglequeue', JSON.stringify({})); } else { $.post('https://vehiclemissions/NUI:togglequeue', JSON.stringify({})); }
    SetQueueButton()
}
...
const RemoveFunction = (id) => {
    ...
    elem.remove()
    $.post('https://vehiclemissions/NUI:deletecontract', JSON.stringify({ 'contract': id }));
    document.getElementById('transfer-contract-container').innerHTML = '';
}
