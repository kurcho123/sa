...
local INPUT_SELECT_NEXT_WEAPON = 16
local INPUT_SELECT_PREV_WEAPON = 17
local INPUT_FRONTEND_RS = 210
...
local settings = {
  ...
  fastMoveMultiplier = 5,
  slowMoveMultiplier = 0.1,
  currentMoveMultiplier = 1,
...
local function SetFreecamEnabled(enable)
  ...
  settings.currentMoveMultiplier = settings.normalMoveMultiplier
  if enable then
    ...
    SetFreecamPosition(coords.x, coords.y, coords.z)
...
Citizen.CreateThread(function()
  local function GetSpeedMultiplier()
    if IsDisabledControlJustReleased(0, INPUT_SELECT_NEXT_WEAPON) then
      if IsDisabledControlPressed(0, INPUT_FRONTEND_RS) then
        settings.currentMoveMultiplier = settings.currentMoveMultiplier + 0.005
      else
        settings.currentMoveMultiplier = settings.currentMoveMultiplier + 0.1
      end
    elseif IsDisabledControlJustReleased(0, INPUT_SELECT_PREV_WEAPON) then
      if IsDisabledControlPressed(0, INPUT_FRONTEND_RS) then
        settings.currentMoveMultiplier = settings.currentMoveMultiplier - 0.005
        if settings.currentMoveMultiplier <= 0 then
          settings.currentMoveMultiplier = 0.005
        end
      else
        settings.currentMoveMultiplier = settings.currentMoveMultiplier - 0.1
        if settings.currentMoveMultiplier <= 0 then
          settings.currentMoveMultiplier = 0.1
        end
      end
    end
    ...
    if settings.currentMoveMultiplier > 15 then
      settings.currentMoveMultiplier = 15
    end

    return settings.currentMoveMultiplier
  end
