...
local curPos
local speeds = {0.001, 0.005, 0.01, 0.02, 0.03, 0.1}
local curSpeed = 1
...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		QBCore.Functions.Notify(Lang:t("info.speed", {value = tostring(speeds[curSpeed])}))
	end
...
RegisterNUICallback('selectOwnedObject', function(data)
	...
	local ownedObject = GetClosestObjectOfType(objectData.x, objectData.y, objectData.z, 1.5, joaat(objectData.hashname), false, 6, 7)
...
RegisterNUICallback("spawnobject", function(data, cb)
	...
	local modelHash = joaat(tostring(data.object))
...
RegisterNUICallback("chooseobject", function(data, cb)
	...
	print('Attempting to load object:', tostring(data.object))
    local modelHash = joaat(data.object)
    if not HasModelLoaded(modelHash) and IsModelInCdimage(modelHash) then
		RequestModel(modelHash)
		local count = 0
		while not HasModelLoaded(modelHash) do
			...
			if count > 10 then
				print('Failed to load model:', data.object)
				break
			end
			count = count + 1
			Wait(500)
	...
	print('Model Loaded', modelHash)
	
	local rotation = GetCamRot(MainCamera, 2)
	local xVect = 2.5 * math.sin( degToRad( rotation.z ) ) * -1.0
    local yVect = 2.5 * math.cos( degToRad( rotation.z ) )
    previewObj = CreateObjectNoOffset(modelHash, curPos.x + xVect, curPos.y + yVect, curPos.z, false, false, false)
    --PlaceObjectOnGroundProperly(previewObj)
	SetModelAsNoLongerNeeded(modelHash)
	print('Object Created', previewObj, DoesEntityExist(previewObj))
		...
		if DecoMode then
			...
			if SelectedObj and peanut then
                ...
                if rotateActive then
					DrawMarker(21, SelObjPos.x, SelObjPos.y, SelObjPos.z + 1.28, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 0.6, 0.6, 0.6, 127, 3, 252, 100, true, true, 2, false, false, false, false)
...
CreateThread(function()
	while true do
		...
		if DecoMode then
			...
			local dist = #(vector3(camPos.x, camPos.y, camPos.z) - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z - Config.MinZOffset))
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
