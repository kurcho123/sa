...
-- Script starts here
local helicam = false
local polmav_hash = `as350`
local medevac_hash = `aw139`
local swatheli_hash = `littlebird`
local drone_hash = `rcmavic`
...
local function IsPlayerInSwatheli()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, swatheli_hash)
end

local function IsPlayerInMedevac()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, medevac_hash)
end

local function IsPlayerInDrone()
	local lPed = PlayerPedId()
	local vehicle = GetVehiclePedIsIn(lPed)
...
RegisterCommand("rappel", function() -- change to radial menu soon
	if IsPlayerInPolmav() or IsPlayerInMedevac()
	or IsPlayerInSwatheli() or IsPlayerInDrone() then
		TriggerEvent("helicam:rappelstart")
	else
		QBCore.Functions.Notify('Can not rappel from this helicopter!', 'error', 7500)
	end
end)

AddEventHandler("helicam:rappelstart", function ()
	local lPed = PlayerPedId()
	...
	if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
		...
		while true do	
			...
			if GetEntityHeightAboveGround(lPed) < 1.0 or IsEntityInWater(lPed) ~= false then									
				break
			end														
		end
		...
		QBCore.Functions.Notify('Can not rappel from this seat!', 'error', 7500)
	end
end)
-- Threads
...
Citizen.CreateThread(function()
	while true do
	  ...
	  or IsPlayerInSwatheli() or IsPlayerInDrone() then
		...
		SetDisableVehicleFlightNozzlePosition(heli, true)
