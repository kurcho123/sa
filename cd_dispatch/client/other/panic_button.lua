if Config.PanicButton.ENABLE then
    ...
    RegisterNetEvent('cd_dispatch:NonEmergencyButtonEvent')
    AddEventHandler('cd_dispatch:NonEmergencyButtonEvent', function()
        local job = GetJob()
        local allowed_jobs = IsAllowed_panic(job)
        if allowed_jobs then
            TriggerServerEvent('cd_dispatch:CallCommand:NonEmergency', GetPlayerInfo(), {job = job, job_table = allowed_jobs, char_name = SourceInfo.char_name})
            PlayAnimation('random@arrests', 'generic_radio_chatter', 1000)
        else
            Notif(3, 'no_perms')
        end
