...
Drilling.HandleControls = function()
  ...
  if last_pos < Drilling.DrillPos then
    if Drilling.DrillSpeed > 0.4 then
      if Drilling.DrillPos >= Drilling.HoleDepth then
        Drilling.DrillTemp = math.min(1.0,Drilling.DrillTemp + ((0.07 * GetFrameTime()) *  (Drilling.DrillSpeed * 10)))
      end
  ...
  elseif Drilling.DrillPos >= 0.78 then
...
RegisterCommand('drilltest', function(source, args)
