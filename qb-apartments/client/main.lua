        ...
        QBCore.Functions.TriggerCallback('apartments:IsOwner', function(result)
            ...
            if isOwned then
                headerMenu[#headerMenu + 1] = {
                    title = Lang:t('text.enter_option'),
            ...
            if PlayerJob.name == "marshalls" or PlayerJob.name == "lspd" or PlayerJob.name == "bcso" or PlayerJob.name == "sast" or PlayerJob.name == "hc" or PlayerJob.name == "pd_cid" then
                headerMenu[#headerMenu+1] = {
                    title = Lang:t('text.raid_apartment'),
                    event = 'apartments:client:PoliceMenu',
                    args = {}
                }
            end
    ...
    local function ShowExitHeaderMenu()
        lib.registerContext({
            ...
            options = {
                ...
                { title = Lang:t('text.leave_option'), event = 'apartments:client:LeaveApartment', },
            }
        })
...
-- Events
    ...
    RegisterNetEvent('apartments:client:OpenStash', function()
        exports.ox_inventory:openInventory('stash', {id = 'apartment', owner = currentApartmentOwner})
    end)
    ...
    AddEventHandler('apartments:client:PoliceMenu', function()
        local input = lib.inputDialog('Raid Apartment', {'Citizen ID (ABC#####)'})
        if not input then return end

        QBCore.Functions.TriggerCallback('apartments:PoliceApartment', function(result)
            if result then
                altaapartment = result.type
                EnterApartment(altaapartment, result.name, false, input[1])
                QBCore.Functions.Notify('Entering ' .. result.name .. ' for Citizen ID: ' .. input[1])
            end
        end, input[1])
    end)
