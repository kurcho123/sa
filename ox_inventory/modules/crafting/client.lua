...
RegisterNetEvent('inventory:client:OpenCraftSet', function(data)
	print("Recieved Craft Set Open")
	QBCore.Functions.Tableprint(data.args)
	if data.args and type(data.args) == 'table' then
		local craftItems = {}
		for id,item in pairs(data.args) do
			craftItems[item] = true
		end
		exports.ox_inventory:openInventory('crafting', craftItems)
	else
		print('Error: No items listed!')
	end
end)
