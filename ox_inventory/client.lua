	...
	if canOpenInventory() then
		...
		elseif inv == 'crafting' then
			...
			if data and data.id then
				...
				left = lib.callback.await('ox_inventory:openCraftingBench', 200, data)
				right = lib.callback.await('ox_inventory:getBlueprintData', 200, data)
				if not data then 
					data = {} 
				else
					data = {craftSet = data}
			...
			if left then
				right = {
					...
					index = data.index or false,
					craftSet = data.craftSet or false,
					slots = right.slots,
					items = right.items,
					coords = coords
				}
		...
		if left then
			...
			if invOpen == false then 
				print('Error: no access 3', invOpen, left)
				lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') }) 
			end
		...
		if success then
			...
			if item.status then
				if client.setPlayerStatus then
					client.setPlayerStatus(data.name, item.status)
				end
			end
		...
		elseif currentWeapon then
			if data.ammo then
				...
				useItem(data, function(resp)
					...
					local addAmmo = GetMaxAmmoInClip(playerPed, currentWeapon.hash, true)
			...
			if not canOpenInventory() then
				...
				if currentInventory then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if plyState.PermsGroup.staff.admin or plyState.PermsGroup.staff.headadmin or plyState.PermsGroup.staff.director then
						else
...
lib.callback.register('ox_inventory:startCrafting', function(id, recipe, count)
	print('Craft count:', count)
	local time = recipe.duration or 7000
	time = time * count
	if time > 45000 then time = 45000 end
	return lib.progressBar({
		label = locale('crafting_item', recipe.metadata?.label or Items[recipe.name].label),
		duration = recipe.duration or 7000,
		canCancel = true,
		disable = {
			move = true,
			combat = true,
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	local success, response = lib.callback.await('ox_inventory:craftItem', 200, id, index, data.fromSlot, data.toSlot, data.count, currentInventory.craftSet)
...
CreateThread(function ()
	...
	exports.ox_inventory:displayMetadata({
		...
		label = 'Label',
		weapon_serial = 'Serial number',
		fingerprint = 'Fingerprint', 
		dnalabel = 'DNA ID',
		bloodtype = 'Blood Type',
		ammolabel = 'Caliber',
		ammotype = 'Type Number',
		street = 'Location',
		purity = 'Purity',
		boxid = 'Serial number',
		amount = 'Amount',
		...
		oxygen = 'Oxygen',
		proof = 'Alcohol Proof',
		repairTime = 'Repair Time Remaining',
	})
end)