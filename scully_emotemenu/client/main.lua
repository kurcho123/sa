...
function EmoteMenu.Play(data, variation)
    if Entity(cache.ped).state.isDead then return end -- dead
    ...
    if data.Scenario then
        ...
        EmoteMenu.IsPlayingAnimation = data.Scenario
        return
    end
    ...
    EmoteMenu.IsPlayingAnimation = {dictionaryName, animationName}
    ...
    if duration and not data.Options.Flags?.KeepProp then
        Wait(duration)
...
function EmoteMenu.CancelAnimation()
    ...
    if EmoteMenu.IsPlayingAnimation then
        ...
        if Config.PtfxKeybind then EmoteMenu.Keybinds.PlayPtfx:disable(true) end
        if EmoteMenu.IsPlayingAnimation[1] and EmoteMenu.IsPlayingAnimation[2] then
            StopAnimTask(cache.ped, EmoteMenu.IsPlayingAnimation[1], EmoteMenu.IsPlayingAnimation[2], 1.0)
        else
    ...
    EmoteMenu.Keybinds.Ragdoll = lib.addKeybind({
        ...
        onPressed = function(key)
            ...
            while EmoteMenu.IsRagdoll do
                Wait(0)
                if Entity(cache.ped).state.isDead then
                    EmoteMenu.IsRagdoll = false
                elseif IsPedOnFoot(cache.ped) then
...
-- Events
RegisterNetEvent('scully_emotemenu:injuryWalk', function(walkname)
    if walkname then
        EmoteMenu.overrideWalk = walkname
        lib.requestAnimSet(EmoteMenu.overrideWalk, 1000)
        SetPedMovementClipset(cache.ped, EmoteMenu.overrideWalk, 1.0)
        ResetPedWeaponMovementClipset(cache.ped)
        ResetPedStrafeClipset(cache.ped) 
    else
        EmoteMenu.overrideWalk = false
    end
end)

RegisterNetEvent('scully_emotemenu:refreshWalk', function()
    if EmoteMenu.overrideWalk ~= false then
        lib.requestAnimSet(EmoteMenu.overrideWalk, 1000)
        SetPedMovementClipset(cache.ped, EmoteMenu.overrideWalk, 1.0)
        ResetPedWeaponMovementClipset(cache.ped)
        ResetPedStrafeClipset(cache.ped) 
...
RegisterNetEvent('animations:client:EmoteCommandStart', function(args)
    if args[1] == 'c' then
        ...
        EmoteMenu.PlayByCommand(args[1], args[2])
    end
end)
