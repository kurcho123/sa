...
CreateThread(function()
	...
	-- addon male
	addNewVariation("hair", "male", 79, 43, true)
	addNewVariation("hair", "male", 80, 43, true)
	addNewVariation("hair", "male", 81, 43, true)
	addNewVariation("hair", "male", 82, 43, true)
	addNewVariation("hair", "male", 83, 43, true)
	addNewVariation("hair", "male", 84, 43, true)
	addNewVariation("hair", "male", 85, 43, true)
	addNewVariation("hair", "male", 86, 43, true)
	addNewVariation("hair", "male", 87, 43, true)
	addNewVariation("hair", "male", 88, 43, true)
	addNewVariation("hair", "male", 89, 43, true)
	addNewVariation("hair", "male", 90, 43, true)
	addNewVariation("hair", "male", 91, 43, true)

	-- female hair
	addNewVariation("hair", "female", 1, 42, true)
	addNewVariation("hair", "female", 2, 42, true)
	addNewVariation("hair", "female", 7, 42, true)
	addNewVariation("hair", "female", 9, 42, true)
	addNewVariation("hair", "female", 10, 42, true)
	addNewVariation("hair", "female", 11, 42, true)
	addNewVariation("hair", "female", 14, 42, true)
	addNewVariation("hair", "female", 15, 42, true)
	addNewVariation("hair", "female", 21, 42, true)
	addNewVariation("hair", "female", 23, 42, true)
	addNewVariation("hair", "female", 31, 42, true)
	addNewVariation("hair", "female", 39, 42, true)
	addNewVariation("hair", "female", 40, 42, true)
	addNewVariation("hair", "female", 42, 42, true)
	addNewVariation("hair", "female", 45, 42, true)
	addNewVariation("hair", "female", 48, 42, true)
	addNewVariation("hair", "female", 49, 42, true)
	addNewVariation("hair", "female", 52, 42, true)
	addNewVariation("hair", "female", 53, 42, true)
	addNewVariation("hair", "female", 54, 42, true)
	addNewVariation("hair", "female", 59, 42, true)
	addNewVariation("hair", "female", 59, 42, true)
	addNewVariation("hair", "female", 68, 42, true)
	addNewVariation("hair", "female", 76, 42, true)
...
-- Toggle Clothing
local function ToggleClothing(which, extra)
	if type(which) == "table" then
    	which = tostring(which.id)
	end
	Wait(50)
	...
	if which ~= "Mask" then
		if not Gender then return end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		QBCore.Functions.Notify("No options available!", "error")
        return
	else
		if not LastEquipped[which] then
...
RegisterNetEvent('radialmenu:client:ToggleClothing', ToggleClothing)
...
function ToggleProps(which)
	...
	if not Prop.Variants then
		if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
			...
			if Last then
				PlayToggleEmote(Prop.Emote.On, function() SetPedPropIndex(Ped, Prop.Prop, Last.Prop, Last.Texture, true) end) LastEquipped[which] = false 
				return
			end
		end
		QBCore.Functions.Notify("You don't appear to have anything to remove!", "error") 
		return
	else
		local Gender = IsMpPed(Ped)
		if not Gender then return end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		QBCore.Functions.Notify("There don't seem to be any variants for this!", "error") 
		return
	end
end
...
-- surgery clothes
local InSurgerySelection = false

CreateThread(function()
	exports['polyzonehelper']:AddBoxZone("InSurgerySelection", vector3(331.62, -588.1, 43.66), 7.0, 8.0, {
        name="InSurgerySelection",
        heading = 252.81,
        debugPoly = false,
		minZ = 43.0,
		maxZ = 45.0
...
AddEventHandler('polyzonehelper:enter', function(name)
    if LocalPlayer.state["isLoggedIn"] then
        if name == "InSurgerySelection" then
			if not added then
				lib.addRadialItem({
					{
					id = 'surgery_clothes',
					...
					icon = 'tshirt',
					menu = 'surgery_clothes_menu'
					}
				})

				lib.registerRadial({
					id = 'surgery_clothes_menu',
					items = {
					{
					...
					{
						label = 'Surgery Clothes',
						icon = 'tshirt',
						onSelect = function()
							TriggerEvent('raid_clothes:client:loadOutfit', nil, "surgery")
						end
					}
					}
				})
				added = true
				InSurgerySelection = true
			end
        end  
    end
end)
...
AddEventHandler('polyzonehelper:exit', function(name)
    if LocalPlayer.state["isLoggedIn"] then
        if name == "InSurgerySelection" then
            if InSurgerySelection then
                lib.removeRadialItem('surgery_clothes')
                InSurgerySelection = false
				added = false
            end
        end
    end
