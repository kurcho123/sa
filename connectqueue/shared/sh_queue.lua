...
function Queue:IsPriority(ids, src)
    Wait(500)
    local prio = false
    local tempPower, tempEnd = Queue:HasTempPriority(ids)
    local getPerms = exports["prdx_playercontroller"]:getPlayerPerms(src)
    if getPerms ~= nil then
        if not getPerms.staff then getPerms.staff = {} end
        if getPerms.staff.director and prio == false then
            --Queue:DebugPrint("This user is a director")
            prio = 100
        elseif getPerms.staff.headadmin and prio == false then
            --Queue:DebugPrint("This user is a head admin")
            prio = 99
        elseif getPerms.staff.admin and prio == false then
            prio = 75
            --Queue:DebugPrint("This user is an admin")
        elseif getPerms.staff.staff and prio == false then
            prio = 50
            --Queue:DebugPrint("This user is staff")
        elseif getPerms.serverOne.vip3 and prio == false then
            prio = 40
           -- Queue:DebugPrint("This user is a VIP III")
        elseif getPerms.serverOne.vip2 and prio == false then
            prio = 35
            --Queue:DebugPrint("This user is a VIP II")
        elseif getPerms.serverOne.vip1 and prio == false then
            prio = 30
            --Queue:DebugPrint("This user is a VIP I")
        elseif (getPerms.serverTwo.lspd or getPerms.serverTwo.bcso or getPerms.serverTwo.sast or getPerms.serverTwo.hc or getPerms.serverTwo.marshalls or getPerms.serverTwo.pd_cid) and prio == false then
            prio = 29
            --Queue:DebugPrint("This user is a LEO")
        elseif getPerms.serverTwo.ems and prio == false then
            prio = 29
            --Queue:DebugPrint("This user is a Fire/EMS")
        elseif getPerms.serverTwo.dispatch and prio == false then
            prio = 29
            --Queue:DebugPrint("This user is a Dispatcher")
        elseif getPerms.serverOne.tow and prio == false then
            prio = 28
            --Queue:DebugPrint("This user is a TOW")
        elseif getPerms.staff.moderator and prio == false then
            prio = 20
            --Queue:DebugPrint("This user is a Support Team")
        elseif getPerms.serverOne.guide and prio == false then
            prio = 20
            --Queue:DebugPrint("This user is a Support Team")
        elseif getPerms.serverOne.veteran and prio == false then
            prio = 10
            --Queue:DebugPrint("This user is a Veteran Player")
        end
    end
    Citizen.Wait(250)
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    local tmp = {
        ...
        ids = ids,
        name = name,
        priority = Queue:IsPriority(ids, src) or (src == "debug" and math_random(0, 15)),
        timeout = 0,
        deferrals = deferrals,
...
function Queue:GetIds(src)
    ...
    if ids and #ids > 1 then
        for k, id in ipairs(ids) do
            if string_sub(id, 1, 3) == "ip:" and not Queue:IsPriority({id}, src) then table_remove(ids, k) end
        end
    end
...
function Queue:AddPriority(id, power, temp)
    ...
    local power2 = tostring(power)
    local temp2 = tostring(temp)
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local function checkforpriority()
    ...
    --     CancelEvent()
    --     Queue:DebugPrint("Dropped " .. name .. ", no discord linked")
    --     return
    -- elseif not getPerms.serverOne.member then
    --     done(Config.Language.wlonly)
    --     CancelEvent()
    --     Queue:DebugPrint("Dropped " .. name .. ", is not a member on discord")
    --     return
    -- end
    ...
    if Config.PriorityOnly and not Queue:IsPriority(ids, src) then done(Config.Language.wlonly) return end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if Queue.DisplayQueue then
            if qCount > 0 then
                SetConvar("queueCount", qCount)
                SetConvar("Queue", qCount)
                SetConvar("queueCountClient", qCount)
            end
            if Queue.InitHostName then
                SetConvar("sv_projectName", (qCount > 0 and "Queue: " .. tostring(qCount) .. " " or "") ..  "^6Paradox Gaming RP ^0| ^2Serious RP")
...
AddEventHandler("rconCommand", function(command, args)
    if command == "cqueue" and commands[args[1]] then
        command = args[1]
        table_remove(args, 1)
