...
function ToggleUI()
    ...
    if (not bMenuOpen) then
        ...
        TriggerServerEvent('qb-banking:server:BankClosed')
    else
...
AddEventHandler("qb-banking:client:UpdateTransactions", function()
    if (bMenuOpen) then
        ...
        QBCore.Functions.TriggerCallback("qb-banking:server:GetBankData", function(data, transactions)
            SendNUIMessage({type = 'update_transactions', transactions = json.encode(transactions), name = charinfo.firstname.. " ".. charinfo.lastname})
            local PlayerBanks = json.encode(data)
            SendNUIMessage({type = "refresh_balances", accounts = PlayerBanks, cash = cash})
        end)
    end
end)
