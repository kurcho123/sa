...
AddEventHandler('qb-banking:client:atm:openUI', function(data) -- this opens ATM
	if not bMenuOpen then
		TaskTurnPedToFaceEntity(PlayerPedId(), data.entity, 2000)
		Wait(2000)
		TriggerEvent('animations:client:EmoteCommandStart', {"atm"})

		QBCore.Functions.Progressbar("atm", "Opening ATM", 2000, false, true, {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
