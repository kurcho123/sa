...
function UpdateAccount(divId, bal, cash)
{
    $("#" + divId + "_Balance").html("$" + bal.toLocaleString());
    var money = cash.toLocaleString();
    $("#accounts-header").text(`Accounts (Cash: $${money})`)
}
...
function AddAccount(account_name, account_type, bal, ply_Name, citizenid)
{
    ...
    if (account_name == "Personal Account")
    account_name = "Personal Account (" + citizenid + ")";
    
    let transferAcc = relang[account_type] || account_type ;
        ...
        <div class='card bgdark1 mb-2'>\
            <div class='card-body'>\
                ...
                <div class='row game-header'>\
                    <div class='col'>\
                        <p class='header-title'> " + (lang[account] || capitalizeFirstLetter(account)) + "</p>\
                    </div>\
                    <div class='col d-flex justify-content-end'>\
                        <p class='header-title'><span class='badge bg-primary'>BANK TRANSACTION</span> | <span class='badge bg-secondary'>" + (lang[why] || why).toUpperCase() + "</span></p>\
                    </div>\
                    <hr/>\
                    <div class='col-2'>\
...
function OpenATM(data, transactions, name, citizenid, cash)
{
    ...
    if (data && data !== null)
    {
        ...
        {
            let tTbl = tbl[i];
            AddAccount((tTbl.type === "business" && tTbl.name || tTbl.type === "organization" && tTbl.name || tTbl.type === "personal" && "Personal Account" || "Checking Account"), (lang[tTbl.type] && lang[tTbl.type] || tTbl.type), tTbl.amount, name, citizenid);
        }
    }
...
Listeners["OpenUI"] = function(data)
{
    ...
    OpenATM(data.accounts, data.transactions, name, citizenid, data.cash);
}
...
Listeners["refresh_balances"] = function(data) {
    ...
    for (var i = 0; i < tbl.length; i++)
    {
        ...
        let account_name = (tTbl.type === "business" && tTbl.name || tTbl.type === "organization" && tTbl.name || tTbl.type === "personal" && "Personal Account" || "Checking Account");
        let temp_name = account_name.replace(/"|'/g,'');
        let temp_Name = account_name.replace(/\s+/g, '');

        if (ValidAccounts[temp_Name])
        {
            UpdateAccount(temp_Name, tTbl.amount, data.cash)
        }
