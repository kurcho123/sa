...
function GetNaked()
	if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
		TriggerEvent('raid_clothes:client:loadOutfit', {
			outfitData = {
...
local function ResetSkin()
	...
	if hairDecor then
		AddPedDecorationFromHashes(ped, hairDecor, hairDecorColor)
	end
end
...
RegisterNetEvent('xnTattoos:setHairDecor', function(data1, data2)
	hairDecor = data1 
	hairDecorColor = data2
end)

RegisterNetEvent('xnTattoos:resetSkin', function()
	Wait(1000)
...
RegisterNetEvent("xnTattoos:SetTattoos", function()
	Wait(1000)
...
CreateThread(function()
    ...
    while true do
		...
		if not IsMenuOpen() then
			...
			for k,v in pairs(Config.ShopsNoInterior) do
