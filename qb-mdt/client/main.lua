...
RegisterNetEvent("prdx_mdt:client:openpublic", function()
    TriggerServerEvent("mdt:server:openMDT")
end)
...
local function EnableGUI(enable, public, noAnim)
    SetNuiFocus(enable, enable)
    SendNUIMessage({ type = "show", enable = enable, job = PlayerData.job.name, boss = (Config.BossPerms[PlayerData.job.name] and Config.BossPerms[PlayerData.job.name][PlayerData.job.grade.level]) or false, rosterLink = Config.RosterLink[PlayerData.job.name] })
    isOpen = enable
    if not public and not noAnim then
        doAnimation()
    end
end
...
RegisterNetEvent('mdt:client:dashboardbulletin', function(sentData)
    QBCore.Functions.TriggerCallback("mdt:server:getBulletins", function(data)
        if data then
            SendNUIMessage({ type = "bulletin", data = data })
        end
    end)
end)
...
RegisterNetEvent('mdt:client:dashboardReports', function(sentData)
    QBCore.Functions.TriggerCallback("mdt:server:getReports", function(data)
        if data then
            SendNUIMessage({ type = "reports", data = data })
        end
    end)
end)

RegisterNUICallback("getAllDashboardData", function(data, cb)
    TriggerEvent("mdt:client:dashboardbulletin")
    if GetJobType(PlayerData.job.name) == "police" then
        TriggerEvent("mdt:client:dashboardWarrants")
    elseif GetJobType(PlayerData.job.name) == "ambulance" then
        TriggerEvent("mdt:client:dashboardReports")
    end
    cb(true)
end)
...
RegisterNetEvent('mdt:client:open', function(bulletin, noAnim)
    if not bulletin then
        EnableGUI(true, true)
    else
        EnableGUI(true, false, noAnim)
    end
    ...
    local welcome = "Welcome, " ..PlayerData.job.grade.name..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2)
    if GetJobType(PlayerData.job.name) == nil then
        welcome = "Welcome"
    end
    SendNUIMessage({ type = "data", activeUnits = activeUnits, name = welcome, location = playerStreetsLocation, fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
    TriggerEvent("mdt:client:dashboardWarrants")
end)
...
RegisterNUICallback("getLogs", function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('mdt:server:GetLogs', function(result)
        p:resolve(result)
    end, data.link)
...
RegisterNUICallback("searchWarrants", function(data, cb)
    ...
    QBCore.Functions.TriggerCallback("mdt:server:getWarrants", function(result)
        p:resolve(result)
    end, data.name)
...
RegisterNUICallback("saveProfile", function(data, cb)
    local profilepic = data.profilepic
    local information = data.description
    local cid = data.cid
    local fName = data.firstname
    local sName = data.lastname
    local tags = data.tags
    local gallery = data.gallery
    local fingerprint = data.fingerprint
    local allergies = data.allergies
    local licences = data.licences

    local differences = {}
    for k, v in pairs(currentProfileData) do
        if data[k] then
            if type(data[k]) == "string" or type(data[k]) == "number" or type(data[k]) == "boolean" then
                if (v ~= "" and data[k] == "") or (v ~= data[k]) then
                    differences[k] = {original = v, new = data[k]}
                end
            elseif type(data[k]) == "table" then
                for x,y in pairs(v) do
                    if y ~= data[k][x] then
                        if not differences[k] then differences[k] = {} end
                        local new = data[k][x]
                        if new == nil then
                            new = ""
                        end
                        differences[k][x] = {original = y, new = new}
                    end
                end
            end
        end
    ...
    for k,v in pairs(data) do
        if type(v) == "string" or type(v) == "number" or type(v) == "boolean" then
            if not currentProfileData[k] then
                if v ~= "" then
                    differences[k] = {original = "", new = v}
                end
            end
        elseif type(v) == "table" then
            for x,y in pairs(v) do
                if not currentProfileData[k] or not next(currentProfileData[k]) then
                    if y ~= "" then
                        if not differences[k] then differences[k] = {} end
                        differences[k][x] = {original = "", new = y}
                    end
                end
            end
        end
    ...
    TriggerServerEvent("mdt:server:saveProfile", data, differences)
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    for i=1,#vehicles do
        vehicles[i]['model'] = GetLabelText(GetDisplayNameFromVehicleModel(vehicles[i]['vehicle']))
    end
...
RegisterNUICallback("deleteIncident", function(data, cb)
    local id = data.id
    TriggerServerEvent('mdt:server:deleteIncident', id)
    cb(true)
end)

RegisterNUICallback("deleteReport", function(data, cb)
    local id = data.id
    TriggerServerEvent('mdt:server:deleteReport', id)
    cb(true)
end)
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    for i=1, #result do
        ...
        result[i]['color'] = Config.ColorInformation[mods['color1']] or "Unknown"
        result[i]['colorName'] = Config.ColorNames[mods['color1']] or "Unknown"
        result[i]['model'] = GetLabelText(GetDisplayNameFromVehicleModel(vehicle['vehicle']))
        local hash = tostring(joaat(vehicle['vehicle']))
        local img = exports.vImageCreator:GetModelImage(hash)
        result[i]['image'] = result[i]['image'] or img
    end
    cb(result)
end)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.inpoundActive then
            ...
            for k,v in pairs(vehicles) do
                local plt = QBCore.Functions.GetPlate(v)
                if plt == plate then
...
RegisterNUICallback("getPenalCode", function(data, cb)
    TriggerServerEvent('mdt:server:getPenalCode', data)
    cb(true)
end)
...
RegisterNUICallback("getStrikes", function(data, cb)
    local p = promise.new()
    QBCore.Functions.TriggerCallback('mdt:server:GetStrikes', function(result)
        p:resolve(result)
    end, data.plate)
    local result = Citizen.Await(p)
    local playerRank = PlayerData.job.grade.level
    if PlayerData.job.name == "marshalls" or PlayerData.job.name == "hc" then
        playerRank = 2
    end
    cb({result = result, playerRank = playerRank})
end)

RegisterNUICallback("removeStrikes", function(data, cb)
    local p = promise.new()
    QBCore.Functions.TriggerCallback('mdt:server:RemoveStrikes', function(result)
        p:resolve(result)
    end, data.plate, data.strike)
    local result = Citizen.Await(p)
    cb(result)
end)

RegisterNUICallback("addStrike", function(data, cb)
    local p = promise.new()
    QBCore.Functions.TriggerCallback('mdt:server:AddStrike', function(result)
        p:resolve(result)
    end, data.plate, data.reason, data.image)
    local result = Citizen.Await(p)
    local playerRank = PlayerData.job.grade.level
    if PlayerData.job.name == "marshalls" or PlayerData.job.name == "hc" then
        playerRank = 2
...
RegisterNetEvent('mdt:client:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        ...
        vehicle['color'] = Config.ColorInformation[vehicle['color1']] or "Unknown"
        vehicle['colorName'] = Config.ColorNames[vehicle['color1']] or "Unknown"
        ...
        local hash = tostring(joaat(vehicle['vehicle']))
        local img = exports.vImageCreator:GetModelImage(hash)
        vehicle['image'] = vehicle['image'] or img
        vehicle['vehicle'] = nil
...
RegisterNetEvent('mdt:client:dashboardMessage', function(sentData)
    local job = GetJobType(PlayerData.job.name)
    if job == "police" or job == 'ambulance' then
