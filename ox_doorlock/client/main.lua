	...
	if double then
		if not door.auto and door.state == 1 then
			while double[1].entity and double[2].entity do
				...
				if math.abs(doorOneHeading - doorOneCurrentHeading) < 0.15 then
				...
				if math.abs(doorTwoHeading - doorTwoCurrentHeading) < 0.15 then
		...
		if not door.auto and door.state == 1 then
			local resetDistance = DoorSystemGetAutomaticDistance(door.hash)
			DoorSystemSetAutomaticDistance(door.hash, 0.0, false, false)
			...
			DoorSystemSetAutomaticDistance(door.hash, resetDistance, false, false)
		end
...
CreateThread(function()
	...
	local lockDoor = 'UNLOCKED'
	local unlockDoor = 'LOCKED'
	local showUI
	...
	while true do
		...
		if ClosestDoor and ClosestDoor.distance < ClosestDoor.maxDistance then
			if Config.DrawTextUI and not ClosestDoor.hideUi and ClosestDoor.state ~= showUI then
				-- shpwUI
				exports['qb-core']:DrawText(ClosestDoor.state == 0 and lockDoor or unlockDoor, 'left')
